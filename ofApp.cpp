...
void ofApp::draw() {
	...
	for (int x = -320; x <= 320; x += 20) {
		...
		for (int y = -320; y <= 320; y += 20) {

			auto deg = ofMap(ofNoise(x * 0.002, y * 0.002 + ofGetFrameNum() * 0.015), 0, 1, -360, 360);
			auto target = glm::vec2(x + cos(deg * DEG_TO_RAD), y + sin(deg * DEG_TO_RAD));

			ofColor fill_color, no_fill_color;

			fill_color = deg > 0 ? ofColor(255) : ofColor(0);
			no_fill_color = deg > 0 ? ofColor(0) : ofColor(0);

			this->draw_arrow(glm::vec2(x, y), target, 10, fill_color, no_fill_color);
		}
	}
}
